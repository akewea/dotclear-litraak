...
litraakPage::check('media,media_admin');

$post_id = !empty($_GET['attachtoid']) ? (integer) $_GET['attachtoid'] : null;
if ($post_id) {
	$post = $litraak->getMilestones(array('milestone_id'=>$post_id));
	if ($post->isEmpty()) {
		$post_id = null;
	}
	$post_title = $post->milestone_title;
...
$page_url = 'plugin.php?p=litraak&popup='.$popup.'&attachtoid='.$post_id;

/*if ($popup) {
	$open_f = array('litraakPage','openPopup');
	$close_f = array('litraakPage','closePopup');
} else {
	$open_f = array('litraakPage','open');
	$close_f = create_function('',"dcPage::helpBlock('core_media'); litraakPage::close();");
}*/
