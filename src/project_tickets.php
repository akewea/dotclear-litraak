...
if (!empty($_POST['action']) && !empty($_POST['tickets']))
{
	...
	try {
		if (preg_match('/^(bug|task|idea)$/',$action))
		{
			...
			$milestone_id = (integer) $action;

			while ($tickets->fetch()) {
				$litraak->updTicketMilestone($tickets->ticket_id,$milestone_id);
			}
		}
	} catch (Exception $e) {
...
$sortby_combo = litraakUtils::getTicketSortByCombo();
$order_combo = litraakUtils::getOrderCombo();

/* Get tickets
 -------------------------------------------------------- */
$milestone = isset($_GET['ticket_milestone']) ?	$_GET['ticket_milestone'] : '';
$type = isset($_GET['ticket_type']) ?	$_GET['ticket_type'] : '';
$status = isset($_GET['ticket_status']) ?	$_GET['ticket_status'] : 'actives';
$sortby = !empty($_GET['ticket_sortby']) ?	$_GET['ticket_sortby'] : 'ticket_status';
$order = !empty($_GET['ticket_order']) ?		$_GET['ticket_order'] : 'asc';
...
# - Milestone filter
if ($milestone !== '' && in_array($milestone,$milestones_combo)) {
...
if ($sortby !== '' && in_array($sortby,$sortby_combo)) {
	...
	if ($sortby != 'ticket_status' || $order != 'asc') {
		$show_filters = true;
	}
}
...
$combo_action[__('Milestone')] = $milestones_combo;
array_shift($combo_action[__('Milestone')]);
...
litraakPage::breadCrumb(' &rsaquo; <a href="'.$litraak->getProjectAdminUrl($project_id).
	'">'.$project_title.'</a> &rsaquo; '.__('Tickets').' - '.
	'<a class="button" href="'.$litraak->getTicketAdminUrl($project_id, '').'">'.
__('New ticket').'</a>');
...
if (!$core->error->flag())
{
	...
	'<div class="col">'.
	'<label>'.__('Milestone:').
	form::combo('ticket_milestone',$milestones_combo,$milestone).'</label> '.
	'<label>'.__('Type:').
	...
	# Show posts
	$post_list->display($page,$nb_per_page,false,
	'<form action="plugin.php?p=litraak&tab=tickets&projectid='.$project_id.'" method="post" id="form-tickets">'.
