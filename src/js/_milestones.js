dotclear.milestoneExpander = function(line) {
	...
	img.onclick = function() {
		dotclear.viewMilestoneContent(this, this.line);
	};
	td.insertBefore(img, td.firstChild);
};
dotclear.viewMilestoneContent = function(img, line) {
	...
	if (!tr) {
		...
		$.get('services.php', {
			...
			if (rsp.attributes[0].value == 'ok') {
				var milestone = $(rsp).find('milestone_desc').text();
				if (milestone) {
					$(td).append(milestone);
				}
			} else {
...
$( function() {
	$('#milestones-table tr.line').each( function() {
		dotclear.milestoneExpander(this);
	});
});